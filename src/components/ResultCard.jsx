  ...
  return (
    <div className='group relative overflow-hidden rounded-2xl bg-slate-900 shadow-[0_14px_40px_rgba(2,6,23,0.35)] ring-1 ring-white/10 transition duration-200 hover:-translate-y-1 hover:shadow-[0_20px_60px_rgba(2,6,23,0.55)]'>
      <a target='_blank' className='block aspect-4/5 w-full' href={item.url} rel="noreferrer">
          {item.type == 'photo' ? <img className='h-full w-full object-cover transition duration-300 group-hover:scale-105 ' src={item.src} alt="" loading="lazy" /> : ''}
          {item.type == 'video' ? <video className='h-full w-full object-cover transition duration-300 group-hover:scale-105' autoPlay loop muted src={item.src} loading="lazy"></video> : ''}
          {item.type == 'gif' ? <img className='h-full w-full object-cover transition duration-300 group-hover:scale-105' src={item.src} alt="" loading="lazy" /> : ''}
      </a>
