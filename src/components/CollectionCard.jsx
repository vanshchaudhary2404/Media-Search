...
const CollectionCard = ({item}) => {
  ...
  const removeFromCollection = (item)=>{
    dispatch(removeCollection(item.id))//this pass item.id match to action.payload and remove 
    dispatch(removeToast())
  }

  return (
    <div className='group relative overflow-hidden rounded-2xl border border-white/10 bg-slate-900 shadow-[0_14px_40px_rgba(2,6,23,0.35)] transition duration-200 hover:-translate-y-1 hover:shadow-[0_20px_60px_rgba(2,6,23,0.55)]'>
      ...
      <div className='absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 bg-linear-to-t from-slate-950/95 via-slate-950/70 to-transparent p-4 text-white'>
        <h2 className='max-h-14 overflow-hidden text-sm font-semibold capitalize leading-5 sm:text-base'>{item.title}</h2>
        <button
          onClick={()=>{
